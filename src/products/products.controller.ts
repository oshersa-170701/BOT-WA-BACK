...
export class ProductsController {
  ...
  // 1. Ocultar listado por POST
  @Post('list')
  async findAllByPhone(@Body() body: { whatsappPhone: string }) {
    return await this.productsService.findAllByPhone(body.whatsappPhone);
  }

  // 2. Ocultar subida de Excel por POST enviando el teléfono en el Body
  @Post('upload-excel')
  ...
  async uploadExcel(
    ...
    try {
      ...
      return { success: true, message: `Se importaron ${importedCount} productos exitosamente.` };
    } catch (error) {
  ...
  @Post(':whatsappPhone')
  create(@Param('whatsappPhone') whatsappPhone: string, @Body() createProductDto: any) {
